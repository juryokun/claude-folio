...
Folio はカーソル移動・キー入力など短時間に何度も実行される操作が多い。「ボタンを押したらすぐ動く」を品質基準とする。

- **ホットパスの `invoke()` 呼び出し**: カーソル移動・選択変更など高頻度操作のたびに Rust 側を呼んでいないか
- **デバウンス・スロットルの要否**: 高頻度イベント（キー連打、スクロール）に対してデバウンスが必要な操作とそうでない操作を区別できているか
- **Zustand サブスクリプションの粒度**: `fileStore` / `tabStore` を過剰に購読して不要な再レンダリングを引き起こしていないか（`useShallow` や個別セレクタを使っているか）
- **大量エントリ処理**: ディレクトリ一覧のフィルタ・ソートなど配列操作が毎レンダーで走っていないか（`useMemo` の適切な使用）
- **Rust 側の同期ブロック**: ファイルシステム操作が async でなく同期 I/O になっていないか（UI スレッドのブロックにつながる）
