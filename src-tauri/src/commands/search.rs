...
const SEVENZIP_CANDIDATES: &[&str] = &["/opt/homebrew/bin/7z", "/usr/local/bin/7z", "/usr/bin/7z"];

const FD_CANDIDATES: &[&str] = &["/opt/homebrew/bin/fd", "/usr/local/bin/fd", "/usr/bin/fd"];
...
pub fn search_with_fd(
    ...
    match fd_type.as_str() {
        "file" => {
            args.extend_from_slice(&["--type", "f"]);
        }
        "dir" => {
            args.extend_from_slice(&["--type", "d"]);
        }
...
#[tauri::command]
pub fn compress_7zip(paths: Vec<String>, dest: String, windows_compat: bool) -> Result<(), String> {
