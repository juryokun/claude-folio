...
export function CommandPalette() {
  ...
  const COMMANDS: CommandDef[] = useMemo(
    () => [
      ...
      { name: 'clear-storage', desc: t('commandPalette.cmd.clearStorage') },
      {
        name: 'lang',
        args: '<ja|en>',
        desc: t('commandPalette.cmd.lang'),
        argValues: ['ja', 'en'],
      },
      {
        name: 'theme',
  ...
  const executeCustomCommand = useCallback(
    async (cmd: (typeof customCommands)[0]) => {
      ...
      const doRun = async () => {
        ...
        try {
          ...
          if (!forceModal && !shouldShowOutputModal(result.stdout)) {
            const brief =
              result.stdout.trim() || t('commandPalette.msg.customDone', { name: cmd.name });
  ...
  const selectArgCandidate = useCallback(
    (val: string) => {
    ...
    <div className="modal-overlay" onClick={close}>
      <div className="modal command-palette" onClick={(e) => e.stopPropagation()}>
        <input
          ...
          placeholder={
            isShellMode ? 'shell command  ({file} {dir} {name})' : t('commandPalette.placeholder')
          }
        />
