...
フローは「振る舞いの変更を伴うか」で判断する。

### 振る舞いの変更を伴う実装（新機能・バグ修正・リファクタリングなど）

1. **PLAN** — `PLAN` エージェントに設計方針を立ててもらい、ユーザーの承認を得る
2. **実装**
3. **REVIEW** — `REVIEW` エージェントにコードレビューを依頼し、指摘事項をすべて対処する
4. **commit** — `/commit` スキルでチェック・テストを通過してからコミットする

### 振る舞いの変わらない変更（テキスト修正・コメント追記・ドキュメント変更など）

1. **実装**
2. **REVIEW** — コードの変更がある場合は必須。ドキュメント・設定ファイルのみの変更は省略可
3. **commit** — `/commit` スキルでチェック・テストを通過してからコミットする

コミットは REVIEW の OK が出るまで行わない。
